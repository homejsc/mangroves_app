...
function dbCopy() {
    var sourceFileName = 'public/' + dbName;
...
function staticsvg(tx) {
    ...
    tx.executeSql("select count(*) as recordcount from flore", [], floreCount, function(tx, error) {
        console.error("Error executing SQL: " + error.message);
    });
    tx.executeSql('select * from hierarchie where C_0 = "Fin"', [], show, function(tx, error) {
        console.error("Error executing SQL: " + error.message);
    });
...
function svgData(temp) {
    ...
    isframe = '<svg id="homesvg" xmlns="http://www.w3.org/2000/svg" xml:space="preserve" shape-rendering="geometricPrecision" text-rendering="geometricPrecision" ' + style_h + ' image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd" viewBox="0 0 2836 2126" xmlns:xlink="http://www.w3.org/1999/xlink">';
    isframe += temp;
    isframe += temp;
    isframe += '</svg>';
...
function familleFunction(tx, familleRes) {
    console.log("Executing famillefunction");
